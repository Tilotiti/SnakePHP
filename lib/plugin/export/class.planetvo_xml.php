...
class planetvo_xml {
    ...
    public function export($stock, $export) {
        ...
        while($stock->next()):
            ...
            if(is_array($auto->get('photo'))):
                foreach($auto->get('photo') as $photo):
                    if(file_exists(WEBROOT.'/images/auto/'.$photo)):
                        ...
                        $md5 .= $photo."    ".md5_file(WEBROOT.'/file/export/tmp/'.$photo)."    \n";
                    endif;
                endforeach;
            elseif($auto->get('photo')):
                if(file_exists(WEBROOT.'/images/auto/'.$auto->get('photo'))):
                    ...
                    $md5 .= $auto->get('photo')."    ".md5_file(WEBROOT.'/file/export/tmp/'.$auto->get('photo'))."    \n";
                endif;
            endif;
            // Choose MAJ or ADD
        ...
        $return = $smarty->fetch('planetvo_xml.tpl');

        $fp = fopen(WEBROOT.'/file/export/tmp/stockvo.xml', "w");
        fputs($fp, $return);
        fclose($fp);
        ...
        @chmod(WEBROOT.'/file/export/tmp/stockvo.xml', 0777);
